...
En la Figura \ref{fig:diagrama_flujo_paralelo} se muestra este flujo paralelo. La configuración global (\texttt{SimulationConfig}) inicializa las tablas estáticas del Cuerpo de Galois una única vez. Posteriormente, el \texttt{BCH\_Simulator} lanza un pool de hilos que ejecutan de manera independiente el pipeline de simulación. Los resultados se sincronizan al final para calcular las métricas agregadas (BER, CWER, tiempos, etc.).
\begin{figure}[h]
    \centering
    \shorthandoff{>}
    \resizebox{\textwidth}{!}{
...
Representa polinomios sobre $GF(2^m)$. Los coeficientes se almacenan en un \texttt{vector<uint16\_t>}, donde el índice corresponde al grado del monomio.
...
Es el orquestador principal del simulador. Gestiona:

\begin{itemize}
    \item La configuración de la simulación (\texttt{SimulationConfig}).
    \item La ejecución de simulaciones Monte Carlo para diferentes valores de $E_b/N_0$.
    \item La paralelización con \textbf{OpenMP} (planificación dinámica).
    \item Criterios de parada inteligentes: mínimo número de \textit{codewords} erróneas o máximo número de iteraciones.    \item La recolección de métricas (BER, BER uncoded, CWER y tiempos de codificación/decodificación).
    \item La exportación de resultados a formato CSV.
\end{itemize}
