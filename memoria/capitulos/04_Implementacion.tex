...
El constructor comprueba que $1\le m\le 15$ y $1 \le 2t < n$. Una vez validados los parámetros, inicializa el cuerpo de Galois, calcula \texttt{n = $2^m - 1$} y determina \texttt{k} a partir del grado del polinomio generador. La función \texttt{computeGeneratorPolynomial()} es la encargada de construir el polinomio generador; para ello, recorre las raíces conjugadas y multiplica los polinomios mínimos correspondientes.
\begin{lstlisting}[style=CodigoCPP]
